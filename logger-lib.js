...
this.Client = function (port, host, options) {
    ...
    // We need to set this option here, in case the module is loaded before `process.title` is set.
    if (! this.options.name) { this.options.name = process.title || process.argv.join(' ') }
    if (this.options.hostname != "") { hostname = this.options.hostname}
    //this.options.pid = this.options.fakepid ?
